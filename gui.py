...
class GUI(tk.Tk, GuiEventProducer):
    ...
    def __init__(self, beeper_strg : BeeperSteuerung, einstellungen_file: Path = Path("einstellungen.txt"), demo_modus=False, *args,**kwargs) -> None:
        super().__init__()
    ...
    def event_button_click(self, event:tk.Event, eventbutton:int):
        ...
        if eventbutton == 2 and event.x > 700 and event.y > 60: 
            ...
            temp_y = min(110, max(90,event.y))  # Position vertikal in das angewaehlte Feld ruecken      
            ...
            if gui_event == GUI_EVENTS.KLINGEL_1:
                ...
                BlockingWindows(self, seconds=3, title=f"Klingel", subtitle=self.Setup_list_namen[0], bild=self.Setup_GlockenBild)

            elif gui_event == GUI_EVENTS.KLINGEL_2:
                #BildPopup(self, seconds=3, x_pos=event.x, y_pos=y_berechnet, bild=self.Setup_GlockenBild) # event.y ist relativ zum jeweiligen "klingelschild" 3x 160px vertikal
                BlockingWindows(self, seconds=3, title=f"Klingel", subtitle=self.Setup_list_namen[1], bild=self.Setup_GlockenBild)
                ...
                BlockingWindows(self, seconds=3, title=f"Öffne Briefkasten", bild=self.Setup_BriefBild)
    ...
    def show_keypad(self):
        ...
        self.keypad = KeyPad(
            ...
            callback_oeffner=lambda evt=GUI_EVENTS.TUER_OEFFNER: self.publish_event(evt),
            callback_shutdown=lambda evt=GUI_EVENTS.SHUTDOWN_EVT: self.publish_event(evt),
...
class Key(tk.Label):
	...
	def __init__(self,master,callback,value = None,*args,**kwargs):
		tk.Label.__init__(self,master,takefocus = True#,bd = 5
						 ,relief = tk.RAISED, width=5, height=3, font=('Helvetica 20 bold'), *args,**kwargs)
...
class KeyPad(tk.Toplevel):
    ...
    def __init__(
        ...
        btn_back = tk.Button(frame, text="Zurück", command=self.destroy, font=('Helvetica 20 bold'))
        ...
        for r in range(1, self.rows+1):
            for c in range(1, self.columns+1):
                num = numbers.pop()
                btn = Key(frame, text="  " + str(num) + "  ", value=str(num), callback=self.digit_clicked).grid(row=r, column=c)
    ...
    def digit_clicked(self, value):
        ...
        if len(self.secret_shutdown_code) > 0:
            ...
            if self.sliding_window_shutdown == self.secret_shutdown_code:
                BlockingWindows(self.master, seconds=4, title="An dieser Stelle kann\n der PI heruntergefahren\n werden")
                # self.master.after(5000, self.destroy)
        ...
        if self.try_counter >= self.secret_max_input_length:
            self.wait_window(BlockingWindows(self, seconds=self.secret_timeout, title="Wird verarbeitet ..."))
            self.destroy()


class BlockingWindows(tk.Toplevel):

    def __init__(self, master,seconds:int=5, title:str = "Wartebildschirm", subtitle:str="", bild:tk.PhotoImage=None, bg_color:str="#222222", fg_color:str="#EEEEEE") -> None:
...
class BildPopup(tk.Toplevel):
    ...
    def __init__(self, master, bild:PhotoImage, x_pos, y_pos, seconds=3, x_anim_offset_px = 10) -> None:
        super().__init__(master=master)
        self.master = master
        ...
        self.config(cursor="none", background=background_color, )

        self.seconds = seconds

        # self.attributes('-alpha', 0.2)

        self.frame = tk.Frame(self, width=self.sidelength, height=self.sidelength, background=background_color)
        self.frame.pack(fill="both", expand=True)

        self.label = tk.Label(self.frame, text=" ", image=bild, background=background_color, foreground="#EEEEEE", font=('Helvetica 50 bold'))
    ...
    def animate(self, anim_recall_ms:int=10, anim_range_px:int=100):
        ...
        if not self.destroy_Lock.locked():
            ...
            if self.anim_pos_toggle:
                # self.x_offset += 1
                self.x_offset += (target_offset_plus - self.x_offset) * (1 - math.exp(- speed_dt))
                if self.x_offset > target_offset_plus-1:
                    self.anim_pos_toggle = False 
            else:
                # self.x_offset -= 1
                self.x_offset += (target_offset_minus - self.x_offset) * (1 - math.exp(- speed_dt))
                if self.x_offset < target_offset_minus+1:
                    self.anim_pos_toggle = True 
                    
            self.label.place(anchor="nw", x=self.x_offset, y=0, relheight=1.0, relwidth=1.0)
                 


            # Animation: Alpha
            alpha_reduction_factor = anim_recall_ms / (self.seconds*1000) # Fuer lineares Alpha in z.B. 3 Sekunden von 1.0 auf 0.0 bei 10 ms Aufrufen

            self.animation_alpha = max(0.0, self.animation_alpha - alpha_reduction_factor + (time.time_ns()- self.anim_start)/self.anim_start)
